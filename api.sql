...
DROP TABLE IF EXISTS `full_semanas`;
CREATE TABLE IF NOT EXISTS `full_semanas` (
  `precio`			decimal(25,2),
  `producto_id` 	varchar(225),
  `sucursal_id`		varchar(225)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_spanish_ci;
...
ALTER TABLE full_semanas ADD CONSTRAINT `producto_fk_semana4` FOREIGN KEY (Producto_id) REFERENCES producto(Producto_id) ON DELETE RESTRICT ON UPDATE RESTRICT;
ALTER TABLE full_semanas ADD CONSTRAINT `sucursal_fk_semana4` FOREIGN KEY (sucursal_id) REFERENCES sucursal(sucursal_id) ON DELETE RESTRICT ON UPDATE RESTRICT;
...
SELECT s.sucursal_id , AVG(k.precio) as PS413 , k.producto_id, s.provincia
FROM sucursal s
left JOIN full_semanas k ON (s.sucursal_id = k.sucursal_id)
where s.sucursal_id = "9-1-688";